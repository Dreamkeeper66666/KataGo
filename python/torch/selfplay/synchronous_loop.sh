...
BATCHSIZE=16 # KataGo normally uses batch size 256, and you can do that too, but for lower-end GPUs 64 or 128 may be needed to avoid running out of memory.
SHUFFLE_MINROWS=700000
...
do
    ...
    #echo "Train"
    #time "$GITROOTDIR"/python/torch/selfplay/train.sh "$BASEDIR" "$TRAININGNAME" "$MODELKIND" "$BATCHSIZE" main --samples-per-epoch "$NUM_TRAIN_SAMPLES_PER_CYCLE"

    ##sleep 30m 
    echo "Export"
    (
        cd "$GITROOTDIR"/python/torch
        time ./selfplay/export_model_for_selfplay.sh "$NAMEPREFIX" "$BASEDIR" "$MODELKIND"
    )
